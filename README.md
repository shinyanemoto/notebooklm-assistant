...
6. 整理/統合（新UI）
   - 全ソースエクスポート用テンプレプロンプトをコピー
   - カスタム役割プロンプト（伴走くん）をコピー
   - ソースまとめ入力欄からテキスト保存（`.txt`）
7. バックアップ出力
   - `notebooklm_chat_export_YYYYMMDD_HHMMSS.md`（チャット経由の全ソース出力）
   - `source_summary_*.txt`（まとめテキスト保存）
...
### 2. 整理・統合（新UI）
1. 「整理/統合」ボタンを押す
2. 「全ソーステンプレをコピー」または「伴走くんテンプレをコピー」を実行
3. NotebookLMチャットへ貼り付けて回答を取得
4. 回答を「ソースのまとめ」欄へ貼り付け
5. 「まとめをテキスト保存（.txt）」でローカル保存

### 3. バックアップ
1. 全件退避は「チャット経由で全ソース出力→MD保存」を実行
   - 拡張が定型プロンプトをチャットに送信
   - 最終回答をMarkdownとして保存
   - 失敗時は定型プロンプトをクリップボードへフォールバック
2. まとめ文は「まとめをテキスト保存（.txt）」を実行
